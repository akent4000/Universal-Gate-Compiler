...
* [ ] **Don't-Care V2 — устранение реальной soundness gap:** V2.b/V2.c/V2.d инструментация и adaptive-W прогон на EPFL (ctrl/dec/cavlc/int2float/router/priority/i2c/adder/bar/sin) показали, что safety-net miter стабильно срабатывает на `router`, `priority`, `i2c`, `sin` и **не лечится поднятием sim-W**: все 5 ретраев до W=16384 тоже ревертятся — значит ревёрты вызваны не sim-undersampling'ом, а теоретическим зазором V2 admissibility check под reconvergent fanout, а не coverage. Adaptive-W выпилен из дефолта (оставлен как `adaptive_sim=True` флаг для отладки), MAX_RETRIES снижен до 2. Остаётся **настоящая проблема**: сформулировать и залатать конкретный V2-инвариант, который ломается. Кандидаты для анализа: (a) `_propagate_care_sim` использует `sig_old` как значение на edge'ах, но после апстрим-переписываний фактические edge-сигналы в `new_aig` могут отличаться на ~care-битах — отсюда дрейф downstream care'а; (b) fallthrough-admissibility считается один раз до enumerate_cuts, но `best_admissible_choice` ищется из тех же кандидатов, что и `best_choice` — возможно, нужен отдельный admissibility-only проход на bust; (c) reconstruct_from_old повторно использует `lit_map` по `oid*2`, который может быть перезаписан более раннего узла с сигнатурой не-matching по care. Требует либо формального доказательства, либо пошагового дифф-teardown на router/priority (минимальный revert-воспроизводящий cone).
