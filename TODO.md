...
Завершено: multi-output Espresso, Brayton kernel extraction, Ashenhurst-Curtis / Roth-Karp декомпозиция, SAT-based exact synthesis, рекурсивная Ashenhurst-Curtis, shared-support multi-output декомпозиция. См. [TODO_done.md](TODO_done.md#фаза-2-булевский-и-структурный-синтез).

### Автоматическая иерархическая декомпозиция (bin→BCD→7seg gap)
*Мотивация: монолитный синтез `Binary_to_7seg_0-99` даёт 370 NAND, а ручная композиция `Binary_to_BCD_0-99 + 2×Binary_to_7seg` — 215. Цель — чтобы компилятор сам восстанавливал промежуточную шину (BCD) и переиспользовал общие подфункции между выходами.*
...
* [ ] **(3) Bi-decomposition (OR/AND/XOR) в rewrite:** Расширить [`rewrite.py`](nand_optimizer/rewrite.py) за пределы 4-cut: для каждого узла с `|support| > 4` искать разбиение `f = g_1 op g_2` (op ∈ {AND, OR, XOR}) c **непересекающимися** или **слабо пересекающимися** (overlap ≤ 1 переменная) supports. Алгоритм: строить матрицу cofactor-разностей по парам входов, кластеризовать переменные в две группы, проверять SAT-solver'ом, что разложение точное (или точное mod don't-cares). Хорошо ловит «две независимые половины» — типа tens vs ones в 7seg-декодере для 0–99, arithmetic bit-slices. Литература: Mishchenko et al., «Fast Boolean Matching for Small Practical Functions» (DAC'09), раздел 4.
**Статус:** Реализовано в [TODO_done.md](TODO_done.md#фаза-2-булевский-и-структурный-синтез) пункт (2) shared-support; пункты (3)–(5) ожидают Opus-классификации.
* [ ] **(4) BDD-based декомпозиция через «узкие сечения»:** Использовать уже подключённый `dd` package (в `requirements.txt`). Построить ROBDD для каждого выхода (или shared ROBDD для группы), найти **узлы с низкой width** (малое число BDD-nodes на данном уровне переменных) — это автоматические границы блоков. Формально: для каждой перестановки переменных считать профиль width(level); глобальные минимумы — кандидаты на ввод промежуточных сигналов `h_i = BDD_above(level_min) output bits`. Комбинировать с sifting-reordering для минимизации профиля. Самый мощный из предложенных методов, но дорогой по времени (`O(|BDD| × n!)` в наивной форме, `O(|BDD|²)` с sifting); включать только на средних задачах (`n ≤ 16`) или по флагу `--bdd-decomp`.
* [ ] **(5) SAT-based resubstitution >4 входов:** Сейчас [`rewrite.py`](nand_optimizer/rewrite.py) ограничен 4-cut'ами из-за размера `AIG_DB_4`. Добавить проход, который для узла `v` с k-cut (k ∈ {5,6,7}) выбирает до m ≤ 3 «дивизоров» — существующих узлов в TFI-окне — и SAT-solver'ом ищет реализацию `v = F(div_1..div_m)` с числом гейтов `< MFFC(v)`. Cost model та же, что уже стоит в rewrite. Это та же идея, что в `dc2` в ABC. Позволит FRAIG'у косвенно склеивать «похожие» подсхемы разных выходов через общие дивизоры. Работает поверх Don't-Care V2 (admissibility check уже есть).

**Приоритет:** (1) + (2) должны закрыть большую часть разрыва на PLA-бенчмарках со структурированной семантикой (binary↔BCD↔7seg, адресные декодеры, мультиплексоры). (3)–(5) — следующая волна для арифметики и криптографии.
...
* [x] **care_rounds_internal как флаг `dc -C N`:** Реализовано в [nand_optimizer/script.py](nand_optimizer/script.py) — добавлен парсер флага `-C N` в команду `dc`, который маппируется в параметр `rounds` функции `dc_optimize()`. Использование: `dc -C 2` для 2 раундов итеративной care-propagation, или `dc -C 3 --odc -K 5` в комбинации с другими флагами. Коммит `f232c33`.
...
* [ ] **Don't-Care V2 — уточнения и инструментация:** V2 ядро работает и soundness защищена тремя слоями (admissibility check → resub/reconstruct fallback → safety-net miter), но остались артефакты и теоретические зазоры, которые либо имеет смысл измерить, либо прямо выпилить:
    * [ ] **(b) Инструментация и динамическое окно resubstitution:** Текущий `resub_window=64` в [`_scan_resub_1gate`](nand_optimizer/dont_care.py) — глобальный cap на перебор пар signals. В AIG >1k узлов хорошие resub-кандидаты могут оказаться вне окна, что даёт потерю rewrite-возможностей без предупреждения. Сделано: (1) в `last_dc_stats()` добавлены счётчики `n_resub_1gate_examined` (пары, пройденные в hot-loop) и `n_resub_1gate_dropped_by_window` (пары, отброшенные именно из-за усечения до `max_window`); (3) FIFO-окно заменено на **topology-aware window** — при превышении пула сохраняются `resub_window` узлов, чей уровень в `new_aig` ближе всего к уровню `old_id` в reference AIG (через вычисленные на лету `level_new` / `_compute_levels_old`). Осталось: (2) прогнать EPFL combinational benchmarks (требует пункт Фазы 5 про AIGER I/O) и замерить реальный hit-rate потерянных кандидатов — решит, нужно ли поднимать дефолтный `resub_window`. (4) Ещё точнее — окно по пересечению TFI-конусов (сигналы, действительно достижимые из фаининов `old_id`); упирается в стоимость вычисления TFI на каждом узле, включать только если (2) покажет, что topology-aware window всё ещё оставляет заметную долю kept-but-irrelevant сигналов.
    * [ ] **(c) Fixed-point care propagation (многораундовая итерация):** [`_propagate_care_sim`](nand_optimizer/dont_care.py) делает single-pass reverse-топологический проход. Для reconvergent paths это даёт **корректное, но не максимально-тугое** care-множество. Mishchenko 2009 §3 описывает single-pass как baseline; tighter-вариант — fixed-point итерация до стабилизации всех `care[v]`. Флаг `dc -C N` (реализован выше) управляет числом раундов. Включать итерации только если замеры QoR на EPFL покажут, что single-pass упирается — подозрительные бенчмарки: `voter`, `router`, `priority encoder`, арифметика с cross-bit reconvergence.
