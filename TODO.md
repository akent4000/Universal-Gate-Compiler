...
* [ ] **Structural choice nodes (ABC `compress2rs`-style):** * Сейчас `rewrite; fraig; rewrite; balance` — последовательно разрушающие проходы: каждый уничтожает структуру предыдущего. Идея choice nodes (Mishchenko et al., "DAG-aware AIG rewriting", DAC 2006; "Integrating Logic Synthesis, Technology Mapping, and Retiming", ICCAD 2006): для каждой функции хранить *несколько* альтернативных AIG-реализаций (snapshot'ы после baseline / balanced / rewritten / fraig'ed проходов), связанных через equivalence chain (`choice_next: node_id → node_id`). На этапе cut-matching rewriter и mapper выбирают лучшую альтернативу per cut, а не per global pass. В ABC `compress2rs` даёт +10-25% поверх single-pass rewrite на индустриальных схемах. Реализация поверх существующих `snapshot()` / FRAIG-классов — оценочно 300-500 LOC в [`core/aig.py`](nand_optimizer/core/aig.py) + новый proc `choice` в script.py. **Наиболее крупный из неисследованных алгоритмических рычагов.**
